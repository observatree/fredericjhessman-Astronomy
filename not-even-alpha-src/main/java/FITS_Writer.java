...
/** This plugin saves a 16 or 32 bit image in FITS format. It is a stripped-down version of the SaveAs_FITS 
	...
	Modified to save additional FITS header information from image info (F.V. Hessman, 2008-FEB-12)
*/
public class FITS_Writer implements PlugIn {
	...
	public void run(String path) {
		...
		if (getHeaders(imp,numImages) == 1)
			{
			getHeader(imp,numImages,0);
	...
	/**
	 * Retrieves FITS headers stored in an ImagePlus.
	 */
	int getHeaders (ImagePlus imp, int num)
		{
		int n=0;
		String info = imp.getProperties().getProperty("Info");
		String hdr = getHeader(info);
		if (num == 1)
			headers = new String[] { hdr };
		else	{
			ImageStack stack = imp.getImageStack();
			String labels[] = stack.getSliceLabels();
			for (int i=0; i < labels.length; i++)
				{
				if (labels[i].contains("SIMPLE  =") && labels(i).contains("END"))
					n++;
				}
			if (n == num)	// FOUND INDIVIDUAL FITS HEADERS FOR EACH IMAGE IN STACK
				{
				headers = new String[num+1];
				headers[0] = blockHeader(imp,num);
				for (int i=1; i <= num; i++)
					headers[i] = extractHeader(labels[i-1]);
				}
			else	{
				headers = new String[] { hdr };
				}
	...
	String blockHeader (ImagePlus imp, int numImages)
		{
		}

	void extractHeader (String label)
		{
		int i=label.indexOf("SIMPLE");
		int j=label.indexOf("END");
		return label.substring(i,j+3);
		}
