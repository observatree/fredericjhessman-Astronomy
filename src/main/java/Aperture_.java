 ...
 * @version 1.14
 * @date 2012-09-20
 * @changes Finished WCS support for modified ApertureRoi (FVH).
 */
public class Aperture_ implements PlugInFilter
	...
	boolean externalWCS = false;
	boolean debug = false;

	double[] raDec = null;
	String[] wcsUnits = null;
	...
	public static String AP_RA = new String("R.A.[deg]");
	public static String AP_DEC = new String("Dec.[deg]");
	...
	protected boolean measureAperture ()
		...
		if (!(externalWCS && raDec != null) && isFITS && wcs.hasRaDec())
			{
			raDec = new double[] { xCenter,yCenter };
			raDec = wcs.pixels2wcs(raDec);
			wcsUnits = wcs.getWCSUnits();
			}
	...
	protected void addApertureRoi ()
		{
		int nrois = ocanvas.numberOfRois()+1;
		if (starOverlay || skyOverlay || valueOverlay)
			{
			ApertureRoi roi = new ApertureRoi (xCenter,yCenter,radius,rBack1,rBack2,source,apertureColor);
			if (valueOverlay)
				roi.setAppearance (starOverlay,skyOverlay,valueOverlay,g.format(source));
			else
				roi.setAppearance (starOverlay,skyOverlay,true,""+nrois);
			roi.setImage (img);
			if ((externalWCS && raDec != null) || (isFITS && wcs.hasRaDec()))
				roi.setWCS(raDec,wcsUnits);
			if (nrois == 1)
	...
	void showApertureStatus ()
		{
		if (isFITS && wcs != null && showRADEC && wcs.hasRaDec() && raDec != null)
			IJ.showStatus (""+slice+": R.A.="+g.format(raDec[0])+", Dec.="+g.format(raDec[1])+", src="
	...
	protected void storeResults ()
		...
		if ((externalWCS && raDec != null) || (isFITS && showRADEC && wcs.hasRaDec() && raDec != null))
			{
			table.addValue (AP_RA, raDec[0], 6);
			table.addValue (AP_DEC, raDec[1], 6);
			}
		if (isFITS && showFits && fitsVals != null)
			{
			String[] sarr = fitsKeywords.split(",");
	...
	protected boolean checkResultsTable()
		...
		if (showPosition && table.getColumnIndex(AP_XCENTER) == ResultsTable.COLUMN_NOT_FOUND)
			i=table.getFreeColumn (AP_XCENTER);
		if (showPosition && table.getColumnIndex(AP_YCENTER) == ResultsTable.COLUMN_NOT_FOUND)
			i=table.getFreeColumn (AP_YCENTER);
		if (showPhotometry && table.getColumnIndex(AP_SOURCE) == ResultsTable.COLUMN_NOT_FOUND)
		...
		if (showBack && table.getColumnIndex(AP_BACK) == ResultsTable.COLUMN_NOT_FOUND)
