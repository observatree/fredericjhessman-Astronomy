 ...
 * @version 1.2
 * @date 2012-SEP-25
 * @changes Upgrade deprecated PlotWindow to Plot.
 */
public class Align_Image implements PlugInFilter
	...
	protected boolean calculateTransformation ()
		...
		if (minimumPoints == SHIFT_ROTATE_SCALE)
			...
			if (lu == null)
				{
				IJ.log("Unable to derive LU decomposition!");
				return false;
				}
			Matrix xMatrix = lu.solve(b);
			if (xMatrix == null)
				{
				IJ.log("Unable to solve x-matrix using LU decomposition!");
				return false;
				}
			...
			if (yMatrix == null)
				{
				IJ.log("Unable to solve y-matrix using LU decomposition!");
				return false;
				}
		...
		if (npts > 2 && plotResiduals)
			...
			Plot pw = new Plot ("Residuals", "X-X(fit) [pixels]", "Y-Y(fit) [pixels]", xx,yy);
			pw.addPoints (xw, yw, Plot.BOX);
