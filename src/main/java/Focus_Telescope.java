 ...
 * LLS object from Astronomy_.jar to do the fittting and also get decent error estimates.
 *
 * @version 1.1
 * @date 2007-SEP-19
 ...
 * @version 1.4
 * @date 2010-MAR-22
 * @changes Fixed bug where table column entries weren't updated.
 * 
 * @version 1.5
 * @date 2010-DEC_01
 * @changes Dynamic update of table availability.
 *
 * @version 1.6
 * @date 2012-SEP-25
 * @changes Upgraded deprecated PlotWindow to Plot, adapted to slightly different LinearFunction interface.
 */
public class Focus_Telescope implements ActionListener, PlugIn
	{
	double[][] x = null;	// USE TO BE JUST double[] BEFORE VERSION 1.6 !!!
	double[] y = null;
	double[] coefs = null;
	int num;
	...
	protected void plotFit ()
		...
		double[] xvec = new double[y.length];
		...
		pw.addPoints (xa,ya, Plot.LINE);
		pw.draw();

		// PLOT DATA

		pw.setColor (Color.BLUE);
		pw.addPoints (xvec, y, Plot.CIRCLE);
		pw.draw();
		pw.show();
	...
	public void actionPerformed (ActionEvent e)
		...
		if (cmd.equals(OK))
			...
			if (Double.isNaN(slope))
				lls.setFunctionType (LLS.HYPERBOLA_FUNCTION, null, null);
			else
				lls.setFunctionType (LLS.FOCUS_FUNCTION, null, new double[] {slope});
			// for (int k=0; k < x.length; k++) IJ.log("\t"+x[k][0]+","+y[k]);
	...
	/*
	 * Creates a sequence of x[][0] and y[] values drawn from a textfield.
	 */
	protected boolean useSimpleDataPairs (String text)
		...
		for (int i=0; i < num; i++)
			{
			try	{
				x[i][0] = Double.parseDouble(vals[2*i]);
		...
		if (nix > 0)
			{
			double[][] xx = new double[num-nix][1];
			double[] yy = new double[num-nix];
			nix=0;
			for (int i=0; i < num; i++)
				{
				if (!Double.isNaN(x[i][0]) && !Double.isNaN(y[i]))
					{
					xx[nix][0] = x[i][0];
					yy[nix] = y[i];
					nix++;
	...
	/*
	 * Creates a sequence of x[][0] focus values for a set of focus y[] values based upon any two of:
	...
	/*
	 * Creates a sequence of x[][0] focus values for a set of focus y[] values based upon an irregular list of values.
	 */
	protected boolean useIrregularSequence (String str)
		...
		try	{
			for (int i=0; i < num; i++)
				x[i][0] = Double.parseDouble (vals[i]);
			}
		catch (NumberFormatException e)
			{
	...
	/*
	 * Creates a sequence of x[][0] focus values derived from the names of the original files.
	 */
	protected boolean useFilename (String prefx, String postfx)
		...
		for (int i=0; i < num; i++)
			...
			if (focus == null)
				return false;
			try	{
				x[i][0] = Double.parseDouble(focus);
				}
			catch (NumberFormatException e)
				{
	...
	protected boolean getOtherColumn (String label)
		...
		xcol = table.getDoubleColumn (widthColumn);
		if (xcol == null)
			{
			IJ.showMessage ("Cannot read column with label \""+label+"\"");
			return false;
			}
		if (xcol.length != y.length)
			{
	...
	protected boolean checkForTables()
		...
		if (tables != null && tables.length > 0)
			...
			for (int n=0; n < tables.length; n++)
				{
				if (tables[n].trim() == "Log" || tables[n].trim() == "B&C")
					tables[n] = null;
				else	{ 
