 ...
 * @version 1.1
 * @date 2012-Sep-20
 * @changes Decimal R.A. in degrees, not hours (FVH)
 *
 * @version 1.2
 * @date 2012-Sep-25
 * @changes Removed Prefs (too complicated!)
 */
public class World_Coordinates implements PlugInFilter
	...
	public void run (ImageProcessor ip)
		...
		if (! wcs.hasWCS())
			{
			IJ.beep();
			IJ.error("Cannot get WCS coordinates - try to repair!");
			img.unlock();
			return;
			}
		...
		proi.setImage (img);
		ocanvas.add (proi);
		String lab = " "+form.dms(radec[0]/15.)+",";	// LABEL IN hh:mm:ss.s
		if (radec[1] > 0.0)
		...
		if (stackSize > 1)
			filename = img.getImageStack().getShortSliceLabel(slice);
		else
		...
		table.addValue (Aperture_.AP_RA, radec[0], 6);
